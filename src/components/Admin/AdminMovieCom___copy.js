...
const AdminMovieCom = ({ list, editMovie, InputChange, EditClick, Update, manualUpdate, show, hide, handleChange, manualinsert, newMovie, delMovie }) => {
    ...
    const getTopMovies = (movies) => {
        ...
        .map(movie => {
            ...
            const movieDate = new Date(date.slice(0,4), date.slice(4,6)-1, date.slice(6,8));
            return {
                ...movie,
                    movieDate,
        ...
        .sort((a, b) => Math.abs(today - a.movieDate) - Math.abs(today - b.movieDate))
        .slice(0, 5);
    };
    ...
    return (
        <div className="admindiv">
            ...
            <div className="admindiv-1">
                ...
                <button className="movieBtn" onClick={() => setShowTop5(!showTop5)}>
                    {showTop5 ? "전체 보기" : "탑 5 보기"}
                </button>
                ...
                <div className="table-wrapper">
                    <table className="movie-table">
                        ...
                        <tbody>
                        {(showTop5 ? getTopMovies(list) : list).length > 0 ? (
                            (showTop5 ? getTopMovies(list) : list).map((data) => (
                                <tr key={data.movieId}>
                            {[
